...
| `publication_state` | Editorial Decision | `'published'` for the 3 verified free recipes. `'draft'` for all other 67 recipes. |
...
| `allergen_review_state` | `allergens` | `'reviewed_listed'` if `cardinality(allergens) > 0`, `'reviewed_no_allergens'` if `cardinality(allergens) = 0`, else `'unknown'`. |
